import { ChowMatcher, ChowOrPongMatcher, Combination, CombinationMatcher, EyePairMatcher, MeldCombinationBuilder, MeldCombinationMatcher, PongMatcher } from "./combi-utils";
...
export namespace StandardCombiMatchers {
  ...
  const FILTER_TERMINAL = (tile: Tile) => tile instanceof NumberedTile && (
    tile.value === 1 || tile.value === 9
  );
  const FILTER_HONOR = (tile: Tile) => tile instanceof HonorTile;
  const FILTER_MIXED_TERMINAL = (tile: Tile) => FILTER_TERMINAL(tile) || FILTER_HONOR(tile);
  ...
  const FILTER_CHARACTERS = (tile: Tile) => tile.suit === StandardMahjong.SUIT_CHARACTERS;
  const FILTER_BAMBOOS = (tile: Tile) => tile.suit === StandardMahjong.SUIT_BAMBOOS;
  const FILTER_DOTS = (tile: Tile) => tile.suit === StandardMahjong.SUIT_DOTS;

  const FILTER_MIXED_CHARACTERS = (tile: Tile) => FILTER_CHARACTERS(tile) || FILTER_HONOR(tile);
  const FILTER_MIXED_BAMBOOS = (tile: Tile) => FILTER_BAMBOOS(tile) || FILTER_HONOR(tile);
  const FILTER_MIXED_DOTS = (tile: Tile) => FILTER_DOTS(tile) || FILTER_HONOR(tile);
  ...
  class CompoundCombinationMatcher implements CombinationMatcher {
    ...
    constructor(readonly combinationName: string, readonly isWinning: boolean, ...matchers: CombinationMatcher[]) {
      this.matchers = matchers;
    }

    getFirstMatch(tiles: Tile[]): Combination | null {
      for (const matcher of this.matchers) {
        const combination = matcher.getFirstMatch(tiles);
        if (combination) {
          return combination;
        }
      }
  ...
  const FILTER_GREEN_SUIT = (tile: Tile) => tile.suit === StandardMahjong.SUIT_DRAGON_GREEN || FILTER_BAMBOOS(tile);
  ...
  export const NON_EXCLUSIVE_MATCHERS = [
    FLUSH,
    TRIPLETS,
    SEQUENCE_HAND,
  ];
...
export const getMatchingCombinations = (tiles: Tile[]): Combination[] => {
  ...
  if (combinations.length === 0) {
    const combination = StandardCombiMatchers.NORMAL_HAND.getFirstMatch(tiles);
    if (combination) {
      combinations.push(combination);
    }
  }
